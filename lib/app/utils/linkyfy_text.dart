...
List<_MatchedString> _findMatches(String text, String types, bool humanize) {
  ...
  if (types.contains('phone')) {
    ...
    for (var matchedBefore in matched) {
      if (matchedBefore.type == MatchType.none) {
        newMatched.addAll(
          _findLinksByType(matchedBefore.text!, MatchType.phone),
        );
      } else {
  ...
  if (types.contains('email')) {
    ...
    for (var matchedBefore in matched) {
      if (matchedBefore.type == MatchType.none) {
        newMatched.addAll(
          _findLinksByType(matchedBefore.text!, MatchType.email),
        );
      } else {
        newMatched.add(matchedBefore);
      }
  ...
  if (types.contains('web')) {
    ...
    for (var matchedBefore in matched) {
      if (matchedBefore.type == MatchType.none) {
        final webMatches = _findLinksByType(
          matchedBefore.text!,
          MatchType.link,
        );
        for (var webMatch in webMatches) {
          if (webMatch.type == MatchType.link &&
            ...
            newMatched.add(
              _MatchedString(
                text: webMatch.text!.substring(
                  webMatch.text!.startsWith('http://') ? 7 : 8,
                ),
                type: MatchType.link,
              ),
            );
...
List<_MatchedString> _findLinksByType(String text, MatchType type) {
  ...
  for (Match match in matches) {
    ...
    output.add(
      _MatchedString(
        type: type,
        text: text.substring(match.start, lastCharacterIndex),
      ),
    );
